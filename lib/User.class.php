...
class User {
    ...
    public static function doLogin($user_name, $password) {
        ...
        if (empty(self::$user_data)) {
            self::$user_data = qs(sprintf("select * from salesperson where (user_name = '%s' OR email = '%s') and password = '%s'", $user_name,$user_name,$password));
        }
...
//        self::$user_data['user_type'] = 'admin';
        User::setSession(self::$user_data);
